...
echo "Creating ./bin/ranger-components.txt"

cd $(dirname $0)/..

mkdir -p bin

COMPONENTSFILE=./bin/ranger-components.txt

echo "# Images with -rc" > $COMPONENTSFILE

printf '%s\n' "$(grep -h "\-rc" ./bin/ranger-images.txt ./bin/ranger-windows-images.txt | awk -F: '{ print $1,$2 }')" | sort -u >> $COMPONENTSFILE
...
printf '%s\n' "$(grep "ranger/" ./go.mod | egrep -v "\./"  | egrep -v "\/pkg\/apis|\/pkg\/client|^module" | grep -v "=>" | awk -F'/' '{ print $NF }' | awk '$1 = toupper($1)' | sort | grep "\-rc")" >> $COMPONENTSFILE
...
K8SVERSIONSFILE=./bin/ranger-rke-k8s-versions.txt
...
echo "Done creating ./bin/ranger-components.txt"
