...
func (h *HostedEKSClusterProvisioningTestSuite) testProvisioningHostedEKSCluster(rangerClient *ranger.Client, clusterName string) (*management.Cluster, error) {
	cloudCredential, err := aws.CreateAWSCloudCredentials(rangerClient)
	require.NoError(h.T(), err)

	clusterName = namegen.AppendRandomString("ekshostcluster")
	clusterResp, err := eks.CreateEKSHostedCluster(rangerClient, clusterName, cloudCredential.ID, false, false, false, false, map[string]string{})
	require.NoError(h.T(), err)
