...
from .test_rke_cluster_provisioning import (create_and_validate_custom_host,
                                            cluster_cleanup)
from .cli_objects import RangerCli
...
SYSTEM_CHART_URL = "https://git.ranger.io/system-charts"
...
# https://github.com/ranger/ranger/issues/25514
...
def test_cli_context_switch(ranger_cli: RangerCli):
    ranger_cli.log.info("Testing Context Switching")
    clusters = ranger_cli.get_clusters()
    client = get_user_client()
    projects = client.list_project()
    assert len(projects) > 0
    for project in projects:
        ranger_cli.switch_context(project['id'])
        cluster_name, project_name = ranger_cli.get_context()
        assert any(cluster["id"] == project['clusterId']
                   and cluster["name"] == cluster_name for cluster in clusters)
        assert project_name == project['name']


def test_cli_project_create(remove_cli_resource, ranger_cli: RangerCli):
    ranger_cli.log.info("Testing Creating Projects")
    initial_projects = ranger_cli.projects.get_current_projects()
    project = ranger_cli.projects.create_project(use_context=False)
    remove_cli_resource("project", project["id"])
    assert project is not None
    assert len(initial_projects) == len(
        ranger_cli.projects.get_current_projects()) - 1


def test_cli_project_delete(remove_cli_resource, ranger_cli: RangerCli):
    ranger_cli.log.info("Testing Deleting Projects")
    initial_projects = ranger_cli.projects.get_current_projects()
    project = ranger_cli.projects.create_project(use_context=False)
    remove_cli_resource("project", project["id"])
    assert project is not None
    ranger_cli.projects.delete_project(project["name"])
    assert len(initial_projects) == len(
        ranger_cli.projects.get_current_projects())


def test_cli_namespace_create(remove_cli_resource, ranger_cli: RangerCli):
    ranger_cli.log.info("Testing Creating Namespaces")
    p1 = ranger_cli.projects.create_project()
    remove_cli_resource("project", p1["id"])
    namespace = ranger_cli.projects.create_namespace()
    remove_cli_resource("namespace", namespace)
    assert len(ranger_cli.projects.get_namespaces()) == 1
    assert "{}|active".format(
        namespace) in ranger_cli.projects.get_namespaces()


def test_cli_namespace_move(remove_cli_resource, ranger_cli: RangerCli):
    ranger_cli.log.info("Testing Moving Namespaces")
    p1 = ranger_cli.projects.create_project()
    remove_cli_resource("project", p1["id"])
    namespace = ranger_cli.projects.create_namespace()
    remove_cli_resource("namespace", namespace)
    ...
    p2 = ranger_cli.projects.create_project(use_context=False)
    remove_cli_resource("project", p2["id"])
    ranger_cli.projects.move_namespace(namespace, p2["id"])
    assert len(ranger_cli.projects.get_namespaces()) == 0
    ranger_cli.projects.switch_context(p2["id"])
    assert len(ranger_cli.projects.get_namespaces()) == 1
    assert "{}|active".format(
        namespace) in ranger_cli.projects.get_namespaces()


def test_cli_namespace_delete(remove_cli_resource, ranger_cli: RangerCli):
    ranger_cli.log.info("Testing Deleting Namespaces")
    p1 = ranger_cli.projects.create_project()
    remove_cli_resource("project", p1["id"])
    namespace = ranger_cli.projects.create_namespace()
    remove_cli_resource("namespace", namespace)
    ...
    assert "{}|active".format(
        namespace) in ranger_cli.projects.get_namespaces()
    deleted = ranger_cli.projects.delete_namespace(namespace)
    assert deleted


def test_cli_app_install(remove_cli_resource, ranger_cli: RangerCli):
    ranger_cli.log.info("Testing Upgrading Apps")
    initial_app = ranger_cli.apps.install(
...
def test_cli_app_values_install(remove_cli_resource, ranger_cli: RangerCli):
    ranger_cli.log.info("Testing Upgrading Apps")
    initial_app = ranger_cli.apps.install(
...
def test_cli_app_upgrade(remove_cli_resource, ranger_cli: RangerCli):
    ranger_cli.log.info("Testing Rolling Back Apps")
    initial_app = ranger_cli.apps.install(
        OPENEBS_CHART, "openebs", version=OPENEBS_CHART_VERSION,
        timeout=APP_TIMEOUT)
    remove_cli_resource("apps", initial_app["id"])
    assert initial_app["version"] == OPENEBS_CHART_VERSION
    upgraded_app = ranger_cli.apps.upgrade(
...
def test_cli_app_rollback(remove_cli_resource, ranger_cli: RangerCli):
    ranger_cli.log.info("Testing Deleting Apps")
    initial_app = ranger_cli.apps.install(
        OPENEBS_CHART, "openebs", version=OPENEBS_CHART_VERSION,
        timeout=APP_TIMEOUT)
    remove_cli_resource("apps", initial_app["id"])
    ...
    upgraded_app = ranger_cli.apps.upgrade(
        initial_app, version=OPENEBS_CHART_VERSION_UPGRADE)
    assert upgraded_app["version"] == OPENEBS_CHART_VERSION_UPGRADE
    rolled_back_app = ranger_cli.apps.rollback(upgraded_app,
                                                OPENEBS_CHART_VERSION)
    assert rolled_back_app["state"] == "active"
    assert rolled_back_app["version"] == OPENEBS_CHART_VERSION


def test_cli_app_delete(ranger_cli: RangerCli):
    ranger_cli.log.info("Testing Deleting Apps")
    initial_app = ranger_cli.apps.install(
        OPENEBS_CHART, "openebs", version=OPENEBS_CHART_VERSION,
        timeout=APP_TIMEOUT)
    deleted = ranger_cli.apps.delete(initial_app)
    assert deleted


def test_cli_app_install_local_dir(remove_cli_resource, ranger_cli: RangerCli):
    ranger_cli.log.info("Testing Installing of an App from Local directory")
    initial_app = ranger_cli.apps.install_local_dir(
...
def test_cli_multiclusterapp_install(custom_cluster, remove_cli_resource,
                                     ranger_cli: RangerCli):
    ranger_cli.log.info("Testing Installing Multi-Cluster Apps")
    ...
    for project in projects:
        if project["name"] == "Default":
            ...
            if cluster_name in [custom_cluster.name, CLUSTER_NAME]:
                ranger_cli.log.debug("Using cluster: %s", cluster_name)
                targets.append(project["id"])
    assert len(targets) > 1

    initial_app = ranger_cli.mcapps.install(
...
def test_cli_multiclusterapp_upgrade(custom_cluster, remove_cli_resource,
                                     ranger_cli: RangerCli):
    ranger_cli.log.info("Testing Upgrading Multi-Cluster Apps")
    # Get list of projects to use and ensure that it is 2 or greater
    client = get_admin_client()
    projects = client.list_project()
    targets = []
    ...
    upgraded_app = ranger_cli.mcapps.upgrade(
        initial_app, version=OPENEBS_CHART_VERSION_UPGRADE,
        timeout=APP_TIMEOUT)
    assert upgraded_app["state"] == "active"
    assert upgraded_app["version"] == OPENEBS_CHART_VERSION_UPGRADE
...
def test_cli_multiclusterapp_rollback(custom_cluster, remove_cli_resource,
                                      ranger_cli: RangerCli):
    ranger_cli.log.info("Testing Rolling Back Multi-Cluster Apps")
    # Get list of projects to use and ensure that it is 2 or greater
    client = get_admin_client()
    projects = client.list_project()
    targets = []
    ...
    rolled_back_app = ranger_cli.mcapps.rollback(
...
def test_cli_multiclusterapp_delete(custom_cluster, remove_cli_resource,
                                    ranger_cli: RangerCli):
    ranger_cli.log.info("Testing Deleting Multi-Cluster Apps")
    # Get list of projects to use and ensure that it is 2 or greater
    client = get_admin_client()
    projects = client.list_project()
    targets = []
    ...
    initial_app = ranger_cli.mcapps.install(
        ...
        timeout=APP_TIMEOUT)
    assert initial_app["version"] == OPENEBS_CHART_VERSION
    deleted, apps_deleted = ranger_cli.mcapps.delete(initial_app)
    assert deleted
    assert apps_deleted


def test_cli_catalog(admin_cli: RangerCli):
...
@if_test_multicluster
def test_cluster_removal(custom_cluster, admin_cli: RangerCli):
    admin_cli.log.info("Testing Cluster Removal")
    deleted = admin_cli.clusters.delete(custom_cluster.name)
    assert deleted


def test_inspection(ranger_cli: RangerCli):
    # Test inspect on the default project used for cli tests
    # Validate it has the expected clusterid, id, type, and active state
    ranger_cli.log.info("Testing Inspect Resource")
    resource = ranger_cli.inspect(
        "project", ranger_cli.default_project["id"],
        format="{{.clusterId}}|{{.id}}|{{.type}}|{{.state}}")
    assert resource is not None
    resource_arr = resource.split("|")
    assert resource_arr[0] == ranger_cli.default_project["clusterId"]
    assert resource_arr[1] == ranger_cli.default_project["id"]
    assert resource_arr[2] == "project"
    assert resource_arr[3] == "active"


def test_ps(custom_workload, ranger_cli: RangerCli):
    ranger_cli.log.info("Testing ranger ps")
    # Deploy a workload and validate that the ps command shows it in the
    # correct namespace with the correct name
    ranger_cli.switch_context(ranger_cli.DEFAULT_CONTEXT)
    ps = ranger_cli.ps()
    expected_value = "{}|{}|nginx|2".format(
        ranger_cli.default_namespace, custom_workload.name)
    assert expected_value in ps.splitlines()


def test_kubectl(custom_workload, ranger_cli: RangerCli):
    ranger_cli.log.info("Testing kubectl commands from the CLI")
    ranger_cli.switch_context(ranger_cli.DEFAULT_CONTEXT)
    jsonpath = "-o jsonpath='{.spec.template.spec.containers[0].image}'"
    result = ranger_cli.kubectl("get deploy -n {} {} {}".format(
        ranger_cli.default_namespace, custom_workload.name, jsonpath))
    assert result == "nginx"


# Note this expects nodes not to be Windows due to usage of ifconfig.me
@pytest.mark.skip(reason="Fails in Jenkins")
def test_ssh(ranger_cli: RangerCli):
    ranger_cli.log.info("Testing ssh into nodes.")
    failures = []
    ranger_cli.switch_context(ranger_cli.DEFAULT_CONTEXT)
    nodes = ranger_cli.nodes.get()
    ranger_cli.log.debug("Nodes is: {}".format(nodes))

    is_jenkins = False
    if os.environ.get("RANCHER_IS_JENKINS", None):
        is_jenkins = True
    for node in nodes:
        ip = ranger_cli.nodes.ssh(node, "curl -s ifconfig.me",
...
def custom_workload(ranger_cli):
    client, cluster = get_user_client_and_cluster()
    project = client.list_project(name=ranger_cli.default_project["name"],
                                  clusterId=cluster.id).data[0]
    p_client = get_project_client_for_token(project, USER_TOKEN)
    workload = p_client.create_workload(
        name=random_str(),
        namespaceId=ranger_cli.default_namespace,
        scale=2,
        containers=[{
            'name': 'one',
            'image': 'nginx',
...
@pytest.fixture(scope='module')
def custom_cluster(request, ranger_cli):
    ranger_cli.log.info("Creating cluster in AWS to test CLI actions that "
...
@pytest.fixture
def admin_cli(request, ranger_cli) -> RangerCli:
    """
    ...
    """
    ranger_cli.login(CATTLE_TEST_URL, ADMIN_TOKEN)

    def fin():
        ranger_cli.login(CATTLE_TEST_URL, USER_TOKEN)
    request.addfinalizer(fin)
    return ranger_cli


@pytest.fixture(scope='module', autouse="True")
def ranger_cli(request) -> RangerCli:
    client, cluster = get_user_client_and_cluster()
    project_id = client.list_project(name='Default',
                                     clusterId=cluster.id).data[0]["id"]
    cli = RangerCli(CATTLE_TEST_URL, USER_TOKEN, project_id)
...
@pytest.fixture
def remove_cli_resource(request, ranger_cli):
    ...
    def _cleanup(resource, r_id):
        def clean():
            ranger_cli.switch_context(ranger_cli.DEFAULT_CONTEXT)
            ranger_cli.log.info("Cleaning up {}: {}".format(resource, r_id))
            ranger_cli.run_command("{} delete {}".format(resource, r_id),
                                    expect_error=True)
